...
def laplaceSNB(target_class, attr, value, feature_idx):  
    laplace = (attribute_count[target_class][attr][value] + 1) / (class_count[target_class] + AttrProbValueNum[feature_idx])
    return laplace

#Selective Naive Bayes
def classifierSNB(run_training_set, run_testing_set, features_to_try):
    ...
    for instance in range(len(run_testing_set[0])):
        ...
        for target_class in range(1,len(class_count)):
            ...
            for attr in range(len(run_testing_set) - 1):
                ...
                likehood *= laplaceSNB(target_class, attr, value, features_to_try[attr])
...
# Selective Naive Byes for Attribute Ranking
# 先選attribute，再5-folds cross vaildation
SNB_ordering_features = []

# 計算先驗機率(Train)矩陣的大小，用來建立while迴圈內的矩陣，另外計算每個attributes的可能值數量 for Laplace
MaxAttrProbValue = 0
...
while True:
    ...
    for feature_index in range(len(Attributes)):
        ...
        if feature_index not in SNB_ordering_features:
            ...
            for i in range(len(folds_data)):
                ...
                acc_count = classifierSNB(run_training_set, run_testing_set, features_to_try)
    ...
    if len(SNB_ordering_features) == len(Attributes):
        break
name_mapping = []
for attr_num in SNB_ordering_features:
    name = column_names[attr_num + 1]
    name_mapping.append(name)
print(f"\nThe ranking result for Selective Naive Bayes :{name_mapping}")

def laplaceNBC(target_class, attr, value):  
    laplace = (attribute_count[target_class][attr][value] + AlphaMatrix[attr][value - 1]) / (class_count[target_class] + sum(AlphaMatrix[attr]))
    return laplace

# Naive Bayes Classifier
def classifierNBC(run_training_set, run_testing_set):
    ...
    for instance in range(len(run_testing_set[0])):
        ...
        for target_class in range(1,len(class_count)):
            ...
            for attr in range(len(run_testing_set) - 1):
                value = int(run_testing_set[attr][instance])
                likehood *= laplaceNBC(target_class, attr, value)
...
# Dirichlet priors with the Laplace’s estimate

# 初始化 Alpha矩陣 for all Attributes
AlphaMatrix = []
for attr in Attributes:
    Alpha = [1 for _ in range(AttrProbValueNum[attr])]
    AlphaMatrix.append(Alpha)

print("\nDirichlet priors with the Laplace estimate:\n")

# 切割5-folds
data_set = fivefolds(discretized_row)

# 依據SNB的順序開始計算每個attribute的Prior Dirichlet priors Alpha_i
for attr_idx in SNB_ordering_features:
    # 先分出training/testing set，再計算出sample mean acc
    for v in range(len(AlphaMatrix[attr_idx])):
        current_acc = 0
        current_Alpha_i = 0
        for w in range(1,51):   #Alpha_i從1~50
            AlphaMatrix[attr_idx][v] = w
            folds_acc_sum = 0 
            for i in range(len(data_set)):
                ...
                acc_count = classifierNBC(run_training_set, run_testing_set)
                fold_acc = acc_count / len(run_testing_set[0])
                folds_acc_sum += fold_acc
            folds_acc_mean = folds_acc_sum / len(data_set)
            if folds_acc_mean > current_acc:
                current_acc = folds_acc_mean
                current_Alpha_i = w
        AlphaMatrix[attr_idx][v] = current_Alpha_i
for a in range(len(AlphaMatrix)):
    print(f"Dirichlet priors for {column_names[a + 1]}: {AlphaMatrix[a]}")

print("\nFolds Accuracy of Dirichlet priors with the Laplace estimate:\n")
for i in range(len(data_set)):
    testing_set = []
    training_set = []
    testing_set.extend(data_set[i])
    ...
    acc_count = classifierNBC(run_training_set, run_testing_set)
    fold_acc = acc_count / len(run_testing_set[0])
    print(f"Correct prediction: {acc_count}, Fold size: {len(run_testing_set[0])}, Accuracy: {acc_count / len(run_testing_set[0]):.4f}")


def noninfo_laplaceNBC(target_class, attr, value):  
    laplace = (attribute_count[target_class][attr][value] + noninfo_AlphaMatrix[attr][value - 1]) / (class_count[target_class] + sum(noninfo_AlphaMatrix[attr]))
    return laplace

def noninfo_classifierNBC(run_training_set, run_testing_set):
    ...
    for instance in range(len(run_testing_set[0])):
        ...
        for target_class in range(1,len(class_count)):
            ...
            for attr in range(len(run_testing_set) - 1):
                value = int(run_testing_set[attr][instance])
                likehood *= noninfo_laplaceNBC(target_class, attr, value)
...
# Best noninformative Dirichlet priors

# 初始化 Alpha矩陣 for all Attributes
noninfo_AlphaMatrix = []
for attr in Attributes:
    Alpha = [1 for _ in range(AttrProbValueNum[attr])]
    noninfo_AlphaMatrix.append(Alpha)

print("\nThe best noninformative Dirichlet priors:\n")
...
for attr_idx in SNB_ordering_features:
    ...
    current_Alpha_i = 0
    for w in range(1,51):   #Alpha_i從1~50
        noninfo_AlphaMatrix[attr_idx] = [w for item in noninfo_AlphaMatrix[attr_idx]]
        folds_acc_sum = 0 
        for i in range(len(data_set)):
            testing_set = []
            training_set = []
            ...
            acc_count = noninfo_classifierNBC(run_training_set, run_testing_set)
            fold_acc = acc_count / len(run_testing_set[0])
            folds_acc_sum += fold_acc
        folds_acc_mean = folds_acc_sum / len(data_set)
        if folds_acc_mean > current_acc:
            current_acc = folds_acc_mean
            current_Alpha_i = w
    noninfo_AlphaMatrix[attr_idx] = [current_Alpha_i for item in noninfo_AlphaMatrix[attr_idx]]
for b in range(len(noninfo_AlphaMatrix)):
    print(f"Dirichlet priors for {column_names[b + 1]}: {noninfo_AlphaMatrix[b]}")

print("\nFolds Accuracy of the best noninformative Dirichlet priors:\n")
for i in range(len(data_set)):
    testing_set = []
    training_set = []
    testing_set.extend(data_set[i])
